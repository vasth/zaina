...
class Easemob {
	 ...
	 *        	自定义参数
	 */
	function Send_pic($from_user = "admin", $username, $content, $target_type = "users", $ext ) {
		$option ['target_type'] = $target_type;
		$option ['target'] = $username;
		$params ['type'] = "img";
		$params ['url'] = "https://a1.easemob.com/easemob-demo/chatdemoui/chatfiles"."9d4dcb10-35c8-11e4-b46f-ad1ce1dbe60a";
		$params ['filename'] = "test1.jpg";
	 	$params ['thumb'] = "https://a1.easemob.com/easemob-demo/chatdemoui/chatfiles"."0c9c0e50-35c9-11e4-940c-257c2aae4c1c";
		$params ['share-secret'] = "DJwOWjXJEeSL-dMZVlBFCzU2l2b-ukAy5mH7HxP1bOesU8no";
		$params ['size'] = array("width" => 640,"height" => 640); 
         $params ['thumb_secret'] =    "";              
	 ...
	 *        	自定义参数
	 */
	function Send_file($from_user = "admin", $username, $content, $target_type = "users", $ext ) {
		$option ['target_type'] = $target_type;
		$option ['target'] = $username;
		$params ['type'] = "file";
		$params ['url'] = "https://a1.easemob.com/ccxt/sandbox/chatfiles/"."1680ce00-35cf-11e4-9b0d-1d5cccca5695";
//		https://a1.easemob.com/easemob-demo/chatdemoui/chatfiles/bca25730-35c6-11e4-b426-effab2241278
		$params ['filename'] = "内测版-1.5.apk";
		$params ['file_length'] = 3854663;
//		$params ['share-secret'] = "p_ky-jXKEeS3nV3F3mda2VERgYnVykiUcFyJQeFTYfQEdHuo";
		$option ['msg'] = $params;
		$option ['from'] = $from_user;
		if(!empty($ext)){
			$option ['ext'] = $ext;
	 ...
	 * @param string $target_type
	 *        	默认为：users 描述：给一个或者多个用户(users)或者群组发送消息(chatgroups)
	 * @param array $content  array('1','2')   //昵称 名片头像地址	
	 * @param array $ext
	 *        	自定义参数
	 */
	function Send_card($from_user = "admin", $username, $content, $target_type = "users", $ext ) {
		$option ['target_type'] = $target_type;
		$option ['target'] = $username;
		$params ['type'] = "file";
		//$params ['url'] = $content; 
		$params ['url'] =  $content['headurl'];
		$params ['filename'] = $content['username'];
		$params ['file_length'] = 3854663; 
		$option ['msg'] = $params;
		$option ['from'] = $from_user;
		if(!empty($ext)){
    ...
    "entities": [
        {
            "uuid": "bca25730-35c6-11e4-b426-effab2241278",
            "type": "chatfile"
        }
    ],
    "timestamp": 1410008841251,
    "duration": 9,
	...
	*/
	 ...
	 *        	默认20
	 */
	public function chatRecord_dao($cursor = '', $limit = 20) {
		 
		$cursor = ! empty ( $cursor ) ? "&cursor=" . $cursor : '';
		$url = $this->url . "chatmessages?limit=" . $limit . $cursor;
		$access_token = $this->getToken ();
		$header [] = 'Authorization: Bearer ' . $access_token;
		$result = $this->postCurl ( $url, '', $header, $type = "GET " );
		return $result;
	 ...
	 *        	 
	 */
	public function set_usernick($user = '', $nickname = '') {
		 
		 $data = array(
			'nickname' => $nickname
		 );
		$url = $this->url."users/".$user;
		$access_token = $this->getToken ();
		$header [] = 'Authorization: Bearer ' . $access_token;
		$result = $this->postCurl ( $url, $data, $header, $type = "PUT " );
		return $result;
	}
