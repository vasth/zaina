...
function select(){
	...
	if($juli == '1'){
		...
		$res_temp = $db->row_select("user","zaina LIKE '$diqu%' $sqlsex ",10,"name,city,province,headurl,sex,age,umd5,zaina");
		foreach($res_temp as $item){
			...
			$start = $page*$pagenum;
			$res_temp = $db->row_select("user","zaina LIKE '$diqu%' $sqlsex ","$start,$pagenum","name,city,province,headurl,sex,age,umd5,zaina");
			foreach($res_temp as $item){
